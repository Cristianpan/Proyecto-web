...
<div class="item-form__section item-form__section--inputs">
    ...
    <?= ($item['isSold'] ?? false) ? '<p class="item-form__disabled">No es posible editar el producto una vez vendido</p>' : '' ?>
    <div class="item-form__field">
        <label for="name" class="item-form__label">Nombre</label>
        <?= validation_show_error('name', 'alert-error') ?>
        <input type="text" class="item-form__input" id="name" name="name" value="<?= old('name') ?? $item['name'] ?? '' ?>" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?>  placeholder="Titulo de la obra" required>
    </div>

    <?= validation_show_error('artStyleId', 'alert-error') ?>
    <?= validation_show_error('artTypeId', 'alert-error') ?>
    <div class="item-form__fields">
        <select name="artTypeId" class="item-form__input item-form__input--select" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?>  required>
        ...
        <select name="artStyleId" class="item-form__input item-form__input--select" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?> required>
    ...
    <div class="item-form__field item-form__field--textarea">
        ...
        <textarea class="item-form__input item-form__input--textarea-short" name="shortDescription" id="shortDescription" placeholder="Intruduce tu obra" required maxlength="60" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?>><?= old('shortDescription') ?? $item['shortDescription'] ?? '' ?></textarea>
    </div>

    <div class="item-form__field item-form__field--textarea">
        <label for="description" class="item-form__label">Descripción</label>
        <?= validation_show_error('description', 'alert-error') ?>
        <textarea class="item-form__input item-form__input--textarea" name="description" id="description" required placeholder="Cuéntanos más sobre ella" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?>><?= old('description') ?? $item['description'] ?? '' ?></textarea>
    </div>

    <div class="item-form__field">
        <label for="materials" class="item-form__label">Materiales</label>
        <?= validation_show_error('materials', 'alert-error') ?>
        <input type="text" class="item-form__input" id="materials" name="materials" value="<?= old('materials') ?? $item['materials'] ?? '' ?>" placeholder="Ej: Acrílico, acuarelas" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?> required>
    </div>
    ...
    <div class="item-form__fields--2">
        <div class="item-form__field">
            ...
            <input type="number" step="0.1" class="item-form__input" id="width" name="width" value="<?= old('width') ?? $item['width'] ?? '' ?>" placeholder="100 cm" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?> required>
        </div>
        <div class="item-form__field">
            <?= validation_show_error('height', 'alert-error') ?>
            <input type="number" step="0.1" class="item-form__input" id="height" name="height" value="<?= old('height') ?? $item['height'] ?? '' ?>" placeholder="100 cm" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?> required>
        </div>
    </div>
    ...
    <div class="item-form__fields--2">
        <div class="item-form__field">
            ...
            <input type="text" class="item-form__input" id="origin" name="localOrigin" value="<?= old('localOrigin') ?? $item['localOrigin'] ?? '' ?>" placeholder="Ej: Mérida" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?> required>
        </div>

        <div class="item-form__field">
            <label for="price" class="item-form__label">Precio</label>
            <?= validation_show_error('price', 'alert-error') ?>
            <input type="number" class="item-form__input" id="price" name="price" step="0.1" min="0" value="<?= old('price') ?? $item['price'] ?? '' ?>" placeholder="100.00" <?= ($item['isSold'] ?? false) ? 'disabled' : '' ?>>
        </div>
    </div>
